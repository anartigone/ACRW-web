...
def process_element(element):
    ...
    Recognized elements:
      ...
      - <a id="title**"> produces a new Markdown file named title**.
      - <h1 class="title-frontmatter"> produces an H1.
      - <h1 class="title-section"> produces an H2.
    ...
    # Process <a> with id "title**" to create a new Markdown file.
    if element.name == "a" and element.get("id", "").startswith("title"):
        title_id = element.get("id")
        title_text = element.get_text().strip()
        if title_text:
            # Create a new Markdown file for this title
            title_md_file = os.path.join(current_part_dir, f"{title_id}.md")
            with open(title_md_file, 'w', encoding='utf-8') as f:
                f.write(f"# {title_text}\n\n")  # Use the title text as H1
            return md_text  # Return empty since we created a new file

    # Process <h1> with class "title-frontmatter" as H1
    if element.name == "h1" and "title-frontmatter" in (element.get("class") or []):
        text = element.get_text().strip()
        if text:
            md_text += f"# {text}\n\n"
        return md_text

    # Process <h1> with class "title-section" as H2
    if element.name == "h1" and "title-section" in (element.get("class") or []):
        text = element.get_text().strip()
        if text:
            md_text += f"## {text}\n\n"
        return md_text
...
def epub_to_honkit(epub_file):
    ...
    for item in book.get_items():
        if item.get_type() == ebooklib.ITEM_DOCUMENT:
            ...
            # Build the Markdown content without the H1 chapter title.
            md_content = ""
            ...
            # Extract the first H1 or H2 title from the md_content for the summary
            title_match = re.search(r'^(# .+|## .+)', md_content, re.MULTILINE)
            if title_match:
                summary_title = title_match.group(0).strip().lstrip('# ').strip()  # Remove the Markdown header syntax
            else:
                summary_title = chapter_title  # Fallback to chapter_title if no H1 or H2 found
            ...
            chapter_titles.append(summary_title)  # Use the extracted title for the summary
